<?php
    ...
    Class Auth {
        ...
        public function reg() {
            ...
            $newuser = $this->pdo->query('INSERT INTO users (login, pass, mail, date, lastvisit, ban, adm, live, hp, hung, thirst, fatigue, user_time, user_weather, user_temp, loc, loc_explored, costumize, craft_lvl, refuge) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            array($this->name_reg, $this->pass_reg, $this->mail_reg, time(), 0, 0, 0, 3, 100, 0, 0, 0, 36000, 1, 2, 1, 0, 0, 1, 0));
        ...
        public function enter() {
            ...
            $find_user = $this->pdo->fetch('SELECT * FROM `users` WHERE `login` = ?  AND `pass` = ?', array($this->name_ent, $this->pass_ent));
